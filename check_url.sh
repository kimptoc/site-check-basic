...
rm -fr WGET_TEMP
wget --tries=2 --timeout=20 -q -PWGET_TEMP $URLTOTEST 2>&1 >/dev/null

result=$?

if [ "$result" -ne "0" ]; then
        echo "Oh-oh - url not found! $URLTOTEST, sending email to $SUPPORTEMAIL"
	echo "To: $SUPPORTEMAIL \n Date `date`:: Error checking for url : $URLTOTEST :: THIS SEEMS TO BE DOWN" | mail -s "Error checking for url : $URLTOTEST" $SUPPORTEMAIL
fi
